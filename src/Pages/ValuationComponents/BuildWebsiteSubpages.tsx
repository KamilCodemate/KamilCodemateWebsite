...
type Props = {
    ...
    subpageAttachments: File[];
    handleFileAdd: (files: File[]) => void;
    handleFileRemove: (index: number) => void;
};

const BuildWebsiteSubpages: React.FC<Props> = ({ content, handleContentChange, subpageAttachments, handleFileAdd, handleFileRemove }) => {
        ...
        >
            ...
            <div className="subpages-content">
                ...
                <div className="attachments-list" style={{ overflowY: 'auto', maxHeight: '100px', marginTop: '10px' }}>
                    {subpageAttachments.map((file, index) => (
                        <div key={index} className="attachment-item">
                            <span className="attachment-name">{file.name}</span>
                            <GoTrash className="remove-icon" size={20} onClick={() => handleFileRemove(index)} style={{verticalAlign: 'middle', marginLeft: 
                                '5px'
                            }}/>
                        </div>
                    ))}
